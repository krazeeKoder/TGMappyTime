...
- (NSInteger)sectionNumberForCurrentRadians {
    ...
    for (NSInteger i = 0; i < self.sections.count; i++) {
        GASection *section = self.sections[i];
...
- (BOOL)radians:(CGFloat)radians isInSection:(GASection *)section {
    BOOL isInSection = NO;
...
- (void)setupSections {
    ...
    if (self.sectionCount%2 == 0) {
        ...
        UIAlertController *alertController = [UIAlertController alertControllerWithTitle:@"Error"
                                                                                 message:@"Odd number of wheel sections not supported yet."
...
- (void)buildEvenNumberOfSections {
    ...
    for (NSInteger i = 0; i < self.sectionCount; i++) {
        GASection *section = [self sectionWithMidValue:midValue andSectionNumber:i];
...
- (GASection *)sectionWithMidValue:(CGFloat)midValue andSectionNumber:(NSInteger)sectionNumber {
    GASection *section = [GASection new];
    section.midValue = midValue;
